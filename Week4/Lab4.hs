...
--Omars version based on the feedback that this could use improvement
--Checks whether a given relation is transitive.
isTransitive :: Ord a => Rel a -> Bool
isTransitive [] = True
isTransitive s = and [ trans pair s | pair <- s ] where trans (x,y) r = and [ elem (x,v) r | (u,v) <- r, u == y ]

--Checks whether a given relation list is properly converted into
-- a transitive closure variant: isTransitive and contains all elements
-- from the original relation list.
testTrClos :: (Ord a) => Rel a -> Bool
testTrClos [] = True
testTrClos x = isTransitive xClos && and (map (\y -> elem y xClos) x)
	where xClos = trClos x
